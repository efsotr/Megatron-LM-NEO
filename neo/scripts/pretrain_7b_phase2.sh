...
NEO_HYPER_PARAM_ARGS="
    ...
    --lr_stable_steps 0 \
"
# lr_stable_steps == -1: use the default value(lr_decay_steps / (1 + wsd_decay_ratio)))
# lr_stable_steps == -2: use the lr_decay_steps value, no decay stage
# wsd_half_life == -1: use the default value(0.5 * (lr_decay_steps - lr_stable_steps) + 1)
# wsd_decay_ratio only works when lr_stable_steps > 0
...
NEO_TRAINING_ARGS="
    ...
    --exit-interval ${EXIT_ITERS:-${TRAIN_ITERS:-6000}}
"
...
CHECKPOINT_PATH="/workspace/checkpoints/to/be/loaded" ### CHECKPOINT_PATH is the path to the checkpoint to be loaded
CHECKPOINT_PATH_TO_SAVE="${OUTPUT_HOME}/checkpoint/"
