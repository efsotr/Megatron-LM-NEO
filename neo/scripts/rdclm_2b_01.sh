...
export GRAD_ACC_STEPS=${GRAD_ACC_STEPS:-16}
...
NEO_TRAINING_ARGS="
    ...
    --optimizer adam \
    --train-iters ${TRAIN_ITERS:-34416} \
    --exit-interval ${EXIT_ITERS:-${TRAIN_ITERS:-600400}}
"
...
TASK_ID=${TASK_ID:-"rPretrain2b"}
JOB_NAME=NEO_2b_nl${NUM_LAYERS}_tp${TP_SIZE}_pp${PP_SIZE}_mb${MICRO_BATCH}_gb${GLOBAL_BATCH}_gas${GRAD_ACC_STEPS}
...
export DATA_PATH="/data/xunjian_yin/mycode/DCLM/dclm-b1/dclm_reverse/dclm_01_text_document"
...
CMD="srun torchrun $DISTRIBUTED_ARGS $PY_SCRIPT_PATH \
    $NEO_MODELING_ARGS \
    $NEO_HYPER_PARAM_ARGS \
    $NEO_TRAINING_ARGS \
    $NEO_DATA_ARGS \
